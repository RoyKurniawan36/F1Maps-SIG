...
function respond_with_error($errors) {
    echo $errors[0];
    exit;
}

function respond_with_json($data, $status = 200) {
...
function fetch_user_data($pdo) {
    global $userData, $hasActiveSubscription, $subscriptionType, $memberSince, $nextBilling;
    try {
        $stmt = $pdo->prepare("
...
function handle_race_list($pdo) {
    $stmt = $pdo->query("SELECT * FROM races ORDER BY date ASC");
    $races = $stmt->fetchAll(PDO::FETCH_ASSOC);

    foreach ($races as &$race) {
        $race['continent'] = getContinentForCountry($race['country']);
    }

    respond_with_json($races);
}

function handle_save_race($pdo) {
    $id = $_POST["race_id"] ?? "";
    ...
    try {
        if ($id) {
            ...
            respond_with_json(["success" => true, "message" => "Race updated."]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO races (title, full_title, date, location, country, latitude, longitude, map_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute(array_values($data));
            respond_with_json(["success" => true, "message" => "Race added.", "id" => $pdo->lastInsertId()]);
        }
    } catch (PDOException $e) {
        error_log("Save race error: " . $e->getMessage());
        respond_with_json(["success" => false, "message" => "Database error occurred."], 500);
    }
}

function handle_delete_race($pdo) {
    try {
...
function handle_get_polygon($pdo) {
    try {
        $stmt = $pdo->prepare("SELECT polygon_data, color FROM race_polygons WHERE race_id = ?");
        $stmt->execute([$_POST["race_id"]]);
        $polygon = $stmt->fetch(PDO::FETCH_ASSOC);
        respond_with_json($polygon ?: null);
    } catch (PDOException $e) {
        error_log("Get polygon error: " . $e->getMessage());
        respond_with_json(["success" => false, "message" => "Database error."], 500);
    }
}

function handle_save_polygon($pdo) {
    $race_id = $_POST['race_id'] ?? null;
    $polygon_data = $_POST['polygon_data'] ?? null;
    $color = $_POST['color'] ?? '#e10600';

    if (!$race_id || !$polygon_data) {
        respond_with_json(["success" => false, "message" => "Missing race_id or polygon_data."], 400);
    }

    try {
        $sql = "INSERT INTO race_polygons (race_id, polygon_data, color) VALUES (?, ?, ?)
                ON DUPLICATE KEY UPDATE polygon_data = VALUES(polygon_data), color = VALUES(color)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$race_id, $polygon_data, $color]);
        respond_with_json(["success" => true, "message" => "Polygon saved successfully."]);
    } catch (PDOException $e) {
        error_log("Save polygon error: " . $e->getMessage());
        respond_with_json(["success" => false, "message" => "Database error occurred while saving polygon."], 500);
    }
}

function handle_update_subscription($pdo) {
...
function handle_cancel_subscription($pdo) {
    if (!isset($_SESSION["user_id"])) {
        echo "Not logged in.";
        exit;
    }
    ...
    try {
        ...
        echo $stmt->rowCount() > 0 ? "success" : "No active subscription to cancel.";
    } catch (PDOException $e) {
...
function getContinentForCountry($countrySlug) {
    $slugMap = [
        'brazil-flag' => 'Americas', 'canada-flag' => 'Americas', 'mexico-flag' => 'Americas', 'united-states-flag' => 'Americas',
        'austria-flag' => 'Europe', 'belgium-flag' => 'Europe', 'hungary-flag' => 'Europe', 'italy-flag' => 'Europe',
        'monaco-flag' => 'Europe', 'netherlands-flag' => 'Europe', 'spain-flag' => 'Europe', 'united-kingdom-flag' => 'Europe',
        'azerbaijan-flag' => 'Asia', 'bahrain-flag' => 'Asia', 'china-flag' => 'Asia', 'japan-flag' => 'Asia',
        'qatar-flag' => 'Asia', 'singapore-flag' => 'Asia', 'united-arab-emirates-flag' => 'Asia', 'saudi-arabia-flag' => 'Asia',
        'indonesia-flag' => 'Asia', 'south-korea-flag' => 'Asia', 'australia-flag' => 'Australia/Oceania', 'new-zealand-flag' => 'Australia/Oceania',
    ];

    return $slugMap[$countrySlug] ?? 'Unknown';
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["register_ajax"])) handle_registration($pdo);
    if (isset($_POST["login_ajax"])) handle_login($pdo);
    if (isset($_POST["get_races"])) handle_race_list($pdo);
    if (isset($_POST["save_race"])) handle_save_race($pdo);
    if (isset($_POST["delete_race"])) handle_delete_race($pdo);
    if (isset($_POST["get_polygon"])) handle_get_polygon($pdo);
    if (isset($_POST["save_polygon"])) handle_save_polygon($pdo);
    if (isset($_POST["update_subscription"])) handle_update_subscription($pdo);
    if (isset($_POST["cancel_subscription"])) handle_cancel_subscription($pdo);
}
    ...
    <div class="site-header">
      ...
      <nav class="nav-tabs" role="navigation" aria-label="Account navigation">
        <ul>
          ...
          <li>
            <a href="#radius-search" class="pagelinks radius-search">Radius Search</a>
          </li>
          <li>
            <a href="#directions" class="pagelinks directions">Directions</a>
          </li>
          <li>
            <a href="#polygon-drawing" class="pagelinks polygon-drawing">Polygon Tool</a>
          </li>
          <?php endif; ?>
          <?php endif; ?>
        </ul>
    ...
    <div class="site-content">
        ...
        <div class="loginForm custom-form">
          ...
          <form id="loginform" method="post">
            ...
            <div class="forgot-password-link">
              <a href="#" class="pagelinks link"
                onclick="document.getElementById('forgotPasswordOverlay').style.display='block'">Forgot your
                password?</a>
            </div>
            <div class="actions">
      ...
      <section class="wrappers my-account-wrapper" style="display: <?= $isLoggedIn ? 'block' : 'none' ?>;"
        ...
        <div class="accountForm custom-form">
          ...
          <div class="actions" style="flex-direction: column; gap: 1rem">
            ...
            <?php endif; ?>
            <a href="#" id="openProfileEditorBtn" class="btn"
              style="width: 100%; background: #6c757d; color: white; text-align: center;">
              ✏️ Edit Profile
            </a>
      ...
      <section class="wrappers order-history-wrapper" style="display: none">
        <div class="orderHistoryForm custom-form">
          ...
          <div id="logContent"
            style="background:#111; color:#0f0;font-family:monospace;height:400px;      overflow-y:auto;padding:1rem;border-radius:8px;">
            Loading...
          </div>
      ...
      <!-- MODIFIED: Radius Search Wrapper -->
      <section class="wrappers radius-search-wrapper" style="display: none;" aria-labelledby="radius-search-heading">
          <div class="custom-form">
              <h1 id="radius-search-heading" class="form-heading">Search Points by Radius</h1>
              <div class="form-grid" style="margin-bottom: 1rem; grid-template-columns: 1fr 1fr 1fr; align-items: flex-end;">
                  <div class="field">
                      <label for="radius-km">Search Radius (km)</label>
                      <input type="number" id="radius-km" class="field-input" value="1000" />
                  </div>
                  <div class="field">
                      <label for="continent-filter">Filter by Continent</label>
                      <select id="continent-filter" class="field-input" disabled>
                          <option value="all">All Continents</option>
                      </select>
                  </div>
                  <div class="field">
                      <button id="radius-search-btn" class="btn btn-primary" style="width: 100%;">Search</button>
                  </div>
              </div>
              <div id="radius-search-results-count" style="margin-bottom: 1rem; text-align: center; font-weight: bold;"></div>
              <div id="radius-map" class="map-container"></div>
              <div id="radius-search-results-list" class="results-list-container">
                  <p>Perform a search to see results here.</p>
              </div>
          </div>
      </section>

      <!-- MODIFIED: Directions Wrapper -->
      <section class="wrappers directions-wrapper" style="display: none;" aria-labelledby="directions-heading">
        <div class="tool-layout">
          <div class="tool-sidebar">
              <h3>Select Destination(s)</h3>
              <ul id="directions-race-list" class="race-sidebar-list">
                  <!-- Races will be populated by JS -->
                  <li>Loading races...</li>
              </ul>
              <!-- **FIX**: Added clear selection button -->
              <button id="clear-directions-selection-btn" class="btn btn-secondary btn-small">Clear Selection</button>
          </div>
          <div class="tool-main-content">
              <div class="custom-form" style="padding: 0;">
                  <h1 id="directions-heading" class="form-heading">Directions to Race</h1>
                  <div class="actions" style="justify-content: center; margin-bottom: 1rem; display: flex; flex-wrap: wrap; gap: 10px;">
                      <button id="get-directions-from-user-btn" class="btn btn-primary">Route From My Location</button>
                      <button id="get-directions-between-races-btn" class="btn btn-secondary">Route Between Selected Races</button>
                  </div>
                   <p style="text-align: center; font-size: 0.9em; margin: -0.5rem 0 1rem 0; color: #666;">
                        Select one or more races. For "Route Between Races", select at least two.
                    </p>
                  <div id="directions-map" class="map-container"></div>
                  <!-- **FIX**: Container for the route itinerary/details -->
                  <div id="directions-results-list" class="results-list-container">
      ...
      <!-- MODIFIED: Polygon Drawing Wrapper -->
      <section class="wrappers polygon-drawing-wrapper" style="display: none;" aria-labelledby="polygon-drawing-heading">
        <div class="tool-layout">
          <div class="tool-sidebar">
              <h3>Select a Race</h3>
              <ul id="polygon-race-list" class="race-sidebar-list">
                  <!-- Races will be populated by JS -->
                  <li>Loading races...</li>
              </ul>
          </div>
          <div class="tool-main-content">
            <div class="custom-form" style="padding: 0;">
                <h1 id="polygon-drawing-heading" class="form-heading">Polygon Drawing Tool</h1>
                <div class="form-grid" style="grid-template-columns: 1fr 1fr; align-items: flex-end; margin-bottom: 1rem;">
                  <div class="field">
                      <label for="polygon-color">Polygon Color</label>
                      <input type="color" id="polygon-color" value="#e10600" style="width: 100%; height: 40px;" />
                  </div>
                  <div class="field">
                      <button id="save-polygon-btn" class="btn btn-primary" style="width: 100%;" disabled>Save Polygon</button>
                  </div>
                </div>
                <div id="polygon-map" class="map-container"></div>
            </div>
          </div>
        </div>
      </section>
          ...
          <form id="raceForm" class="custom-form">
            ...
            <div class="field">
              ...
              <input type="text" class="field-input" id="title" name="title" required
                onkeydown="if(event.key !== 'Enter') return" />
            </div>
            ...
            <div class="form-grid">
              ...
              <div class="field">
                ...
                <select name="country" id="country" onchange="updateFlagPreview(); updateMapLocation();"
                  onkeydown="if(event.key !== 'Enter') return">
        ...
        <div class="overlay-content forgot-password-wrapper">
          ...
          <form id="forgetPasswordForm" action="/admin/reset_password.php" method="post" novalidate>
            ...
            <div class="field">
              ...
              <input type="password" id="new-password" name="new_password" class="txtPassword"
            ...
            <div class="field">
              <label for="confirm-password">Confirm Password</ label>
                <input type="password" id="confirm-password" name="confirm_password" class="txtPassword"
